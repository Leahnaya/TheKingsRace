...
namespace MLAPI.Relay
{
    public static class Program
    {
                                ...
                                case MessageType.ConnectToServer:
                                    {
                                        ...
                                        IPEndPoint hostEP = hasIPAddress(endpoint);
                                        ...
                                        if (hostEP != null)
                                        {
                                            ...
                                            Room room = ServerAddressToRoom[hostEP];
                                        ...
                                        {
                                            Console.WriteLine("[ERROR] Endpoint not found! -> [" + endpoint + "]");
                                            Console.WriteLine("Size of Dictionary: " + ServerAddressToRoom.Count);
                                            int keyCounter = 1;
                                            foreach (EndPoint ep in ServerAddressToRoom.Keys)
                                            {
                                                Console.WriteLine("[" + keyCounter + "] Key Value is: [" + ep + "]");
                                            }
                                        }
                                    }
                                    break;
        ...
        private static IPEndPoint hasIPAddress(IPEndPoint ep) {
            foreach (IPEndPoint end in ServerAddressToRoom.Keys) {
                Console.WriteLine("Dictionary Entry: " + end.Address.ToString() + " | Connection Address: " + ep.Address.ToString());
                if (end.Address.ToString() == ep.Address.ToString()) {
                    return end;
                }
            }
            return null;
